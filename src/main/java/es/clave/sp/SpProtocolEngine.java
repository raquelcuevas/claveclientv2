...
public final class SpProtocolEngine extends ProtocolEngineNoMetadata implements SpProtocolEngineI {
    ...
    public SpProtocolEngine(@Nonnull final ProtocolConfigurationAccessorNoMetadata configurationAccessor) {
    ...
    @Nonnull
    @Override
    public byte[] checkAndDecryptResponse(@Nonnull final byte[] responseBytes) throws EIDASSAMLEngineException {
        // This decrypts the given responseBytes:
        final CorrelatedResponse response = (CorrelatedResponse) unmarshallResponse(responseBytes);
