...
//        return ((String)ApplicationContextProvider.getApplicationContext().getBean(Constants.SP_REPO_BEAN_NAME)).trim();
    	return Constants.SP_CONFIG_REPOSITORY;
    }
     ...
     */
    public static boolean isEncryptedSamlResponse(final byte[] tokenSaml) throws UnmarshallException {
     ...
     */
    public static String extractAssertionAsString(final String samlMsg) {
        String assertion = NO_ASSERTION;
        try {
            final Document doc = DocumentBuilderFactoryUtil.parse(samlMsg);

            final XPath xPath = XPathFactory.newInstance().newXPath();
            final Node node = (Node) xPath.evaluate(ASSERTION_XPATH, doc, XPathConstants.NODE);
            if (node != null) {
