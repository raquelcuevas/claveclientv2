...
public class UrlHttpManagerImpl implements UrlHttpManager {
	...
	static {
		...
		LOGGER.info("Establecido el gestor de cookies por defecto: " + cookieManager.toString()); //$NON-NLS-1$
	}
	...
	/** EStablece el gestor de <i>cookies</i>.
	 * @param cHandler Gestor de <i>cookies</i>. */
	public static void setCookieManager(final CookieHandler cHandler) {
	...
	public byte[] readUrl(final String urlToRead,
		...
		if (urlParameters != null) {
			final int contentLength = urlParameters.getBytes(StandardCharsets.UTF_8).length;
			conn.setRequestProperty(
				"Content-Length", String.valueOf(contentLength) //$NON-NLS-1$
			);
